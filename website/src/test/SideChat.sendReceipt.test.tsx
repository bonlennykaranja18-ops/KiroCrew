...
describe('SideChat send receipt policy', () => {
  ...
  it('text a refusal handed back to the MOUNTED composer survives an unmount (tab switch) and is back on remount', async () => {
    // The composer draft is component-local; the refusal above merged the text
    // into it because the panel was showing this slot. An activity-tab switch
    // unmounts the panel before the user resends: the draft is parked into the
    // store on unmount and drained back on the next mount, so the standing
    ...
    unmount()
    expect(store.getState().chat.slotSide[SLOT]?.releasedText).toBe('keep me')
    renderWithProviders(<SideChat slot={SLOT} />, { store })
    await waitFor(() => expect(box().value).toContain('keep me'))
    expect(screen.getByRole('alert')).toHaveTextContent('Send failed: side queue is full (max 20)')
    // Drained once: the store copy is cleared, so a second mount cannot duplicate it.
    expect(store.getState().chat.slotSide[SLOT]?.releasedText).toBeUndefined()
  })

  it('an EMPTY composer parks nothing on unmount', async () => {
    const store = sideState(false)
    const { unmount } = renderWithProviders(<SideChat slot={SLOT} />, { store })
    unmount()
    expect(store.getState().chat.slotSide[SLOT]?.releasedText).toBeUndefined()
