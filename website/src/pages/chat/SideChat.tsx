...
export default function SideChat({ slot }: { slot: string }) {
  ...
  // The composer draft is component-local (`useComposerDraft` is uncontrolled
  // here), and an activity-tab switch UNMOUNTS this panel. Text merged into the
  // draft by `recoverFor` above -- the question a failed or unconfirmed send
  // handed back -- would die with the component, while its standing notice in
  // the store kept asserting "your text is back in the composer" over an empty
  // box. Park whatever is in the draft at unmount into the same store channel
  // the mount-time drain below already reads (`releasedText`), so the next
  // panel showing this slot puts it back. An explicit close is not a park: the
  // `sideClose` reducer deletes the slot's side state, and `sideHandBackText`
  // no-ops on a missing slot, so a question the user discarded stays discarded.
  const draftRef = useRef(draft)
  draftRef.current = draft
  useEffect(() => () => {
    const parked = draftRef.current
    if (parked.trim()) dispatch(sideHandBackText({ slot: slotRef.current, text: parked }))
  }, [dispatch])
