 ...
 *  user row appearing past that point -- by identity ONLY, never by text, which
 *  a manual resend or a duplicate injection could share. The server keeps the
 *  id on every path a send can take: a dispatched send persists it on its row
 *  directly, and a send that was queued behind a busy slot carries it on the
 *  queue entry so the drain writes it onto the row (#8853). `undo` is the
 *  composer before/after an unconfirmed restore, so that proof can take the
 *  text back out. */
interface SendTail {
  role: 'error' | 'notice'
  content: string
...
function ChatEmbed({ slotKey, agent, placeholder, frameless, startAtBottom, onSend, aboveComposer }: ChatEmbedProps) {
   ...
   *  row lives here -- and a receipt that lands after this instance unmounted
   *  is parked by slot (`embedSendRecovery`) for the next instance to show.
   *  Cleared when the next send starts. */
  ...
  /** Show a receipt's outcome on THIS embed: the tail row, and the handed-back
   *  text merged into the composer (with the before/after an unconfirmed
   *  restore needs so proven delivery can take it back). One function for the
   *  live path and for a receipt drained after a remount, so the two cannot
   *  render the same outcome differently. */
  const applyRecovery = useCallback((rec: PendingEmbedRecovery) => {
    const undo = rec.restoreText == null ? undefined : restoreIntoComposer(rec.restoreText)
    setSendTail({ ...rec.tail, undo })
  }, [restoreIntoComposer])
  ...
  useEffect(() => {
    // Drain anything a previous instance parked for this slot, and keep
    // listening: a receipt can land AFTER this instance mounted (the user came
    // back within the deadline) but be addressed by the instance that sent it,
    // which by then is unmounted and can only park it.
    const drain = () => { const rec = drainEmbedRecovery(slotKey); if (rec) applyRecovery(rec) }
    drain()
