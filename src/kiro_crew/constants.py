...
#: never eaten, and a TRAILING wrapper only when it abuts the closer (or its
#: ``(...)`` tic) AND the marker itself is line-anchored -- spelled as a
#: conditional on the ``lwrap`` group, which participates (possibly empty) at
#: line start and cannot participate mid-line. A MID-LINE marker keeps the
#: pre-widening grammar exactly: without the condition, the trailing run eats
#: the closing character of a surrounding inline span (``\`Use [OPTIONS: A |
#: B]\```), corrupting the visible prose and emitting bogus buttons. Runs are
#: capped at 3 (``***`` is the longest CommonMark emphasis run; 4+ is not a
#: wrapper).
#:
...
OPTIONS_RE_LINE = re.compile(
    rf"(?P<lwrap>^[ \t]*{_MARKER_WRAP_CLASS}{{0,3}})?"
