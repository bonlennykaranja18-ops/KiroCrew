"""The shared appearance library's HTTP surface: ``/api/appearances/...``.

Crew Companion's gallery already had these operations, but only under
``/api/apps/crew-companion/``, where every handler is wrapped in
``_require_enabled`` and answers 403 while the app is off. A crew that wears a
pack cannot depend on that: disabling an app must not blank the roster. So the
library gets a surface on the dashboard's own router, reading the one shared
store (:mod:`kiro_crew.dashboard.appearances`). The app's routes stay exactly
where they are and now delegate to the same store, so the gallery's contract is
unchanged and the two surfaces can no longer show different libraries.
...
async def require_library_owner(request: web.Request, operation: str) -> web.Response | None:
    """The owner gate for ANY route reaching the shared appearance library.

    Public because Crew Companion's own gallery routes must pass through it too.
    Its routes predate the shared library and were gated only on the app being
    enabled — which was sufficient while the library was that app's private
    directory, and stopped being sufficient the moment the same store started
    deciding what the crew roster draws. An owner-only surface with a second door
    open to any authenticated app token is not owner-only, which is the rule
    ``apps/builtins/aws_control`` states outright: such a door "would hand out
    through one door what is locked behind another".
    ...
    log lets a reader see who was turned away and not who got in.
    """
...
async def _require_owner(request: web.Request, operation: str) -> web.Response | None:
    """This module's alias for :func:`require_library_owner`."""
    return await require_library_owner(request, operation)
...
async def api_appearances_import(request: web.Request) -> web.Response:
    ...
    if error is not None:
        return error
    store = await asyncio.to_thread(get_appearance_store)
    result = await asyncio.to_thread(import_bundle, store, payload)
