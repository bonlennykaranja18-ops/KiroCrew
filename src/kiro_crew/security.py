...
def _home_dir_targets_uncached(
    ...
    resolve: Callable[[str], str | None] = _realpath_or_none,
) -> set[str]:
    """Anchor the ``$HOME``-relative *home_dirs* entries into absolute, casefolded
    on-disk targets.

    *resolve* is the ``realpath`` the per-anchor resolved forms come from.  The
    default touches the filesystem, so in production this function runs on the
    ``mc-pathres`` pool via :func:`_home_dir_targets` (see there for why) and
    only direct callers and tests run it inline; the parameter exists so a test
    can stand in a recording or wedged resolver.
    ...
    *roots* optionally supplies the already-resolved :class:`_ResolvedRoots`
    ...
    home_real = resolve(home) or home
    ...
    if kiro_home_override and _KIRO_AGENTS_DIR in home_dirs:
        ...
        agents_real = resolve(agents_full)
...
def sandbox_credential_targets(exclude_leaves: tuple[str, ...] = ()) -> tuple[str, ...]:
    ...
    *exclude_leaves* drops a ``$HOME``-relative leaf (and its override
    ...
    # (``_sandbox_preflight`` wraps it in ``asyncio.to_thread``), whose stall
    # handling is the transport ladder's, not the path gate's (found in review).
    # A sandbox mask must be canonical whatever the disk is doing, so the
    # worker that resolves the roots for the gate is called here directly and
    # simply waits.
