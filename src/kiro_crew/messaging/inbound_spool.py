...
verbatim. **A route is declared only where ``may_send_to`` can express
...
#: whole budget, and a spooled prompt is replayed verbatim, so truncation is
#: marked in the text rather than done silently.
TEXT_CAP = 16_384
...
def _quote(entry: SpooledInbound, transport: Any) -> str:
    """The restart notice for *entry*, made display-safe for *transport*.
    ...
    carry a broadcast mention, and echoing it back unescaped would fire it.
    """
    body = entry.text
    ...
    notice = RESTART_NOTICE.format(quoted=quoted)
    capabilities = getattr(transport, "capabilities", None)
    if capabilities is None:
        return notice
    try:
        return display_safe_for(notice, capabilities)
    except Exception:
        return notice
...
def _route_authorized(entry: SpooledInbound, transport: Any) -> bool:
    ...
    try:
        return bool(gate(entry.conversation_id, thread, principal=principal))
    except Exception:
        logger.warning(
