...
| [rfc-central-governance-ceiling.md](rfc-central-governance-ceiling.md) | `in-progress` | Steps 1-6 are on this branch: the MDM-managed tier, its root-owned trust check, central-above-env precedence, tighten-only subordinates, and Ed25519 policy signing; the dated `break_glass` override was withdrawn before merge and is a follow-up. Step 7 (server-side attestation, the only step that binds local root) is a documented non-goal |
