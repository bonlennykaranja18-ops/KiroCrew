...
| Appearance library | BACKEND ONLY so far — no user-reachable UI in this release. The install's one library of appearance packs (art plus optional per-state sound cues), rooted at `<data home>/appearance-library/` rather than inside the Crew Companion app, so a crew wearing a pack still renders while that app is disabled. A crew record can name a pack (`avatar: {"kind":"pack","id":...}`) and the API can list, read, import, PetDex-fetch and delete packs; deleting a pack a crew wears is refused by name unless forced. The Crew Companion gallery reads the same library through its own existing routes, so packs are authored there today. The crew-editor picker that lets a user CHOOSE a pack is a separate frontend change and does not exist yet | Not reachable from the dashboard UI yet — API and the Crew Companion gallery only | None yet (`components/CrewAvatarBuilder.tsx` gains the picker in a later change); `apps/crew-companion/` for pack authoring | `handlers/appearances.py`, `dashboard/appearances.py` (shared store), `apps/builtins/crew_companion/appearances.py` (store class) | `GET /api/appearances`, `GET /api/appearances/{id}`, `GET /api/appearances/{id}/slot/{slot}`, `GET /api/appearances/{id}/sound/{state}`, `POST /api/appearances/import`, `POST /api/appearances/petdex/fetch`, `DELETE /api/appearances/{id}` |
