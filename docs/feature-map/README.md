...
| Startup feature video | One short clip introducing a new feature, shown once on the launch after it ships and retired permanently by a `seen` or `dismissed` verdict; yields the whole launch to release notes, an update popup or first-run onboarding, and is skipped in incognito/temporary sessions | Nothing appears yet (pre-integration): the modal and its gate are mounted in `App.tsx`, but no backend serves `/api/feature-videos/*`, so the fetch 404s and the modal renders nothing. Once the endpoints land it appears automatically at startup, with no navigation | `components/StartupVideoModal.tsx`, `components/startupVideoGate.ts` (sequencing policy), mounted app-wide in `App.tsx` | — (no handler yet) | `GET /api/feature-videos/next`, `POST /api/feature-videos/feedback` (the contract the frontend codes against; not served yet) |
