...
| Side chat | A scratch sub-conversation beside the main turn | Chat right panel → **Side** | `pages/chat/SideChat.tsx`, `pages/chat/SidePanel.tsx`, `pages/chat/sideTurnWire.ts` (transport adapter for the send — not a feature) | `handlers/side.py` | `POST /api/chat/slots/{slot}/side/open`, `.../side/turn`, `.../side/close` |
