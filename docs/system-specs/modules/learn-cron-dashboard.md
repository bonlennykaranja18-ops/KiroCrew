...
non-empty shape with 400 `invalid_avatar`, mirroring `session_color`. The **pack tier** is pure config too (`{"kind":"pack","id":...}`) and is specified below under *Shared appearance library*. Any kind
...
**Shared appearance library**: a third avatar tier — `avatar: {"kind":"pack","id":...}` — dresses a crew in an appearance pack, and packs live in ONE library shared by every surface. The store class is Crew Companion's (`apps/builtins/crew_companion/appearances.py::AppearanceStore`, unchanged), but the INSTANCE and its root moved up: `dashboard/appearances.py` owns a lazily-built process-wide store rooted at `<data home>/appearance-library/` behind a `threading.Lock` (the shape `artifacts.get_default_store` already uses), and `crew_companion/hooks.py::get_appearances` now delegates to it instead of building its own from `ctx.data_dir`. Two instances would be two libraries: a pack imported through the gallery would be missing from the crew editor, and one of the two would silently own the colour maps. The store is deliberately NOT warmed by the app's `on_startup` hook — that hook runs inside aiohttp's `runner.setup()`, so anything it awaits is paid before the dashboard socket binds, and a directory scan plus a one-time migration is exactly the maintenance-shaped work the boot path must not carry (`no-new-work-on-gateway-boot-path`); every caller resolves the store INSIDE a worker thread instead, so the first request pays it off the event loop. **Migration** ends on a `.legacy-migrated` marker in the shared library, not on "the shared library has packs in it": those two tests diverge the moment one pack moves and the next one fails, and an emptiness test then skips the migration forever and strands every pack behind the failure. Each pack directory under the app's old `data/appearances/` whose name is a legal pack id is MOVED (`os.replace`; across filesystems it copies to a `.migrating-<id>-<pid>` staging name and renames into place, because a bare `copytree` to the target is not atomic and a part-way failure would leave a partial pack at the real id) and the recolourings travel with it on their own call, so a transient colour-write failure is retried rather than made permanent. The marker is written only once every pack has been moved or found already taken — a collision is an answered question, a failure is not — and its finality is what stops a pack the user deliberately deleted from being resurrected by a later start. A pack that cannot be moved is logged and left exactly where it is, so the worst outcome is a pack that stays invisible until someone reads the log rather than one that is gone. Routes live on the DASHBOARD router (the `agents` slice, after the crew-roster block), not under `/api/apps/crew-companion/`, because every handler there is wrapped in `_require_enabled` and answers 403 while the app is off — which would blank the face of every crew wearing a pack. Every route on the surface, READS INCLUDED, passes through `handlers/appearances.py::require_library_owner`, which is `require_owner_dashboard_request` plus the accepted-decision audit the shared gate does not record (it audits the denial and returns `None` on success, so half a permission decision reached the log). All auditing goes through one non-raising `_audit` chokepoint: an audit describes an operation and must never decide it, so a write that audits after its mutation cannot turn a completed delete into a 500. Crew Companion's own `/api/apps/crew-companion/appearances*` routes now pass through the SAME owner gate (`_require_library_owner`, enabled-check first so a disabled app still answers `app_disabled` to everyone): enabled-only was sufficient while the library was that app's private directory and stopped being sufficient when the same store began deciding what the roster draws, since an owner-only surface with a second door open to any authenticated app token is not owner-only — the rule `aws_control` states as handing out "through one door what is locked behind another". Routes: GET `/api/appearances` → `{packs:[...]}` built-in first; GET `/api/appearances/{id}` → `pack_detail` (404 `pack_not_found` on a miss OR a malformed id — telling the two apart only hands a caller probing for traversal a signal it does not need); GET `/api/appearances/{id}/slot/{slot}` serves ONE slot's bytes with a content-hash `ETag`, `Cache-Control: private, max-age=60` and `X-Resolved-Slot` naming what was actually served, resolving the fallback chain server-side (`working`→`working|loading|thinking|idle`, `done`→`done|idle`, `error`→`error|idle`; any other name resolves to itself only, because a pack's random clips are named by their author and a fixed vocabulary would make them unfetchable, so "unknown slot" means "not in this pack even after fallback" → 404 `slot_not_found`) — `svg` is served as `image/svg+xml` under `Content-Security-Policy: script-src 'none'; style-src 'unsafe-inline'`, the same policy `handlers/files.py` puts on an untrusted SVG read, because a pack's SVG is third-party markup on the dashboard's own authenticated origin and navigating to it renders it as a document (the picture tier refuses SVG outright for this reason; a pack's art must be SVG, so it is served inert instead); `lottie` as `application/json`; a `sprite` sheet base64-DECODED to `image/png`, because this route exists to be an `<img src>` and base64 text under an image content type is not an image; every media answer also carries `X-Content-Type-Options: nosniff`, and the headers ride the 304 as well as the 200. The built-in `kiro-ghost` answers 404 `builtin_no_content` since its art ships inside the frontend bundle. GET `/api/appearances/{id}/sound/{state}`; POST `/api/appearances/import` accepting both the app's own `{"bundle":...}` JSON envelope (so the frontend reuses its client code) and a multipart file part read in 64 KB chunks under `MAX_BUNDLE_BYTES` — the part iteration is guarded too, since a declared boundary that never appears raises there rather than at `request.multipart()` — delegating every check to `pack_transfer.import_bundle`; POST `/api/appearances/petdex/fetch` delegating to the existing fetch with its host allow-list, HTTPS pin, redirect re-validation and download ceiling untouched; DELETE `/api/appearances/{id}` refusing the built-in with 400 `builtin_pack` and refusing 409 `pack_in_use` (body lists the crew names) while any crew's `avatar` names that pack, unless `?force=1`. That guard is `dashboard/appearances.py::delete_pack_if_unworn`, shared by both delete doors, and it canonicalizes the pack id ONCE so the wearer lookup and the store cannot disagree — the store normalizes through `safe_pack_id` while a raw comparison does not, so `aurora ` found no wearer and then deleted `aurora`. The in-use read runs under the agents routes' own config lock (without it a crew save landing between read and delete leaves exactly the reference the check exists to find) and the delete goes through `_drained_to_thread`, so a cancelled request cannot release that lock mid-removal. A forced delete leaves the crews with a dangling reference that renders the name-derived ghost, which is what an absent pack already means. **Pack sounds** (schema only this round — no upload UI): a manifest may carry `"sounds": {"working"|"done"|"error": "<filename>"}` naming a `.mp3`/`.ogg`/`.wav` file inside the pack directory, stored base64-encoded exactly as a sprite sheet is (the store's write path is text-only), capped at `MAX_SOUND_BYTES` (512 KB decoded) and MIME-sniffed from the decoded bytes rather than from the filename — a manifest is hand-editable, and a name is not evidence of what a file is. A junk entry is dropped with a warning and never fatal, so one bad cue costs that cue and not the pack's art. `pack_detail` carries `sounds` as a state→present map so a client never probes, the audio itself is fetched per state, and `export_bundle` reads the cues straight off the store (`pack_sound_payload`) rather than out of the detail payload, since a bundle built from a presence map would carry no audio and an export/delete/import round trip would destroy every cue. The importer applies the same ceiling AND the same sniff the reader will, so a valid-base64 non-audio file is refused where the user can see it instead of importing "successfully" and never playing; it requires at least one `ART_SUFFIXES` file (a non-empty file map stopped meaning "has art" once audio was allowed, so a sound-only bundle installed a pack with nothing to draw) and refuses a manifest naming a cue it does not carry. `save_sprite_pack`'s filename allow-list stays image-only (`ART_SUFFIXES`) so audio cannot be stored as a sprite sheet.
