...
`_TIER_INTERSECTS_AUDITED`) with the tier NAMES only — module constants this process
...
`_ENV_BENEATH_CENTRAL_WARNED` for the process lifetime. An env document governing alone,
...
the one point every central document passes. A delegating fleet that has not
...
- `platform/governance.py` — archetypes, catalog, loader, evaluator
  ...
  `compose_installed_ceiling` (+ `_LAST_BUNDLED`, the remembered bundled tier).
