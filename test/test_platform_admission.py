...
class TestManifestParsing:
    def test_string_capability_value_is_not_exploded(self):
        ...
        # signed payload.
        m = PluginManifest.from_dict({"name": "p", "capabilities": {"egress": "*.amazon.com"}})
...
class TestStrictBooleanFlagCoercion:
    ...
    def test_the_sibling_flags_no_longer_carry_the_string_trap(self, key):
        ...
        This replaces a test that pinned the siblings to the OLD ``bool()`` behaviour so
        that widening would have to be a deliberate decision. It is: the retrospective
        on round 14 chose it. The first widening read a malformed value as OFF and was
        caught in review as a gate failing open (``"true"`` -> unsigned plugin admitted);
        the gate flags now fail CLOSED on a malformed value.
        """
