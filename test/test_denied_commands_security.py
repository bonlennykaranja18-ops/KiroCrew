...
class TestProductNameAnywhereIsNotADenial:
    ...
    Eleven catalog rows fired on exactly that: the four self-management
    subcommand rows opened with an unbounded any-run before the product name, so
    the name in a worktree path plus the verb word anywhere later matched, and
    the seven legacy rows refused a recursive search of the product's own tree
    for an underscore-joined library method name no shell resolves. All eleven
    are deleted rather than narrowed. What must NOT have moved is the structural
    argv floor: it is a separate control, it decides on the argv's program and
    leading subcommand rather than on words, and for the four subcommands it is
    now the whole of enforcement -- so the genuine cases stay denied while every
    spelling of "the name was merely present" is allowed.
    """
