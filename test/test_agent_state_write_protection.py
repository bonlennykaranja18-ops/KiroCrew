...
class TestAgentStateSidecarWriteProtection:
    ...
    def test_bash_write_is_blocked(self) -> None:
        for cmd in (
            "echo forged > ~/.kiro/crew/agent_model_state.json",
            "tee ~/.kiro/crew/agent_model_state.json",
            "cp evil ~/.kirocrew/agent_model_state.json",
        ):
    ...
    def test_sibling_data_home_paths_unaffected(self) -> None:
        ...
        assert is_sensitive_write_path("~/.kiro/crew/config.json") is True  # quick check: list intact
