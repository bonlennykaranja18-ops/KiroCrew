...
class TestPodMintedGrantsAreFencedFromToolCalls:
    ...
    #
    # WHICH gate, after #9089: `is_sensitive_path`, the layer that RESOLVES. The
    # bash-TEXT leg that used to route each token through it is gone, so an
    # absolute override-rooted spelling is no longer refused there -- for
    # KIROCREW_HOME exactly as for KIROCREW_OS_HOME. Pinned as a parity statement
    # by test_the_bash_text_layer_covers_no_relocated_home_after_9089 below.
    ...
    def test_the_bash_text_layer_covers_no_relocated_home_after_9089(
        self, tmp_path: Path, monkeypatch
    ) -> None:
        """The bash-TEXT leg stopped covering this tree -- and not only this one.

        Before #9089 ("move the path fence to the layer that can hold it") the bash
        leg had a second pass that tokenized a command and routed each token through
        ``is_sensitive_path``, so an absolute pod-home spelling was refused there
        too. That pass is deleted. What survives is a literal regex whose home
        anchor is ``Path.home()`` / ``~`` / ``$HOME`` / a generic ``/home/<user>``
        (see ``security._sensitive_path_pattern``), and no override root is among
        its alternatives -- nor can one be, since the compiled pattern is a module
        global with no invalidation hook while an override is read per call.

        So this is a PARITY statement, not a hole this change opened:
        ``KIROCREW_HOME`` -- main's own long-standing relocation override, which
        this branch does not touch -- is equally uncovered at that layer, and both
        roots are refused by the layer #9089 keeps. Asserted in both directions, so
        losing the resolving fence fails loudly and reinstating either bash
        coverage shows up here rather than silently.
        """
        os_home = tmp_path / "pod-os-home"
        ...
        # The layer that does not, at parity between them.
        ...
        # The regex-anchored real home is untouched: the surviving matcher works.
        assert security.is_sensitive_bash_command("cat ~/.aws/credentials") is not None
